...
const BrandLink = ({ compact = false }: { compact?: boolean }) => (
  ...
  >
    <span className={`grid place-items-center rounded-xl ${compact ? "size-10 bg-secondary" : "size-11 bg-white"}`}>
      <Image src={logo.src} alt="" width="44" height="44" className="size-9 object-contain" />
    </span>
    Urban Dwell
  </Link>
);
...
const AuthShell = ({ children, visualLabel, visualTitle, visualDescription }: AuthShellProps) => (
    ...
    <section className="flex min-h-[100svh] overflow-y-auto px-5 py-7 sm:px-10 sm:py-10 lg:h-screen lg:px-12 xl:px-16" aria-label="Account form">
      <div className="m-auto w-full max-w-[31rem]">
        <div className="mb-14 flex items-center justify-between lg:hidden">
          <BrandLink compact />
          <Link href="/" className="inline-flex items-center gap-2 text-sm font-bold text-base-content/60 hover:text-primary">
