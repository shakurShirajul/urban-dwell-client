...
const CheckoutForm = ({ paymentData }: { paymentData: PaymentData }) => {
  ...
  if (!authContext) throw new Error("CheckoutForm must be rendered within AuthProvider");
  const { user, successToast } = authContext;
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const { paymentIntent, error } = await stripe.confirmCardPayment(clientSecret, {
      payment_method: { card, billing_details: { email: user.email, name: user.displayName ?? undefined } },
    });
