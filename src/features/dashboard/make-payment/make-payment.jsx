...
const MakePayment = () => {
  ...
  const continueToPayment = (event) => {
    ...
    navigate("/dashboard/paymentpage", { state: { month: event.currentTarget.month.value, amount: profile.rent } });
  };

  const details = [["Block", profile?.block_name, HiOutlineSquares2X2], ["Floor", profile?.floor_no, HiOutlineBuildingOffice2], ["Apartment", profile?.apartment_no, HiOutlineHomeModern]];
