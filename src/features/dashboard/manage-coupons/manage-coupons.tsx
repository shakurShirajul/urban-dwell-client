  ...
  const createCoupon = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await axiosSecure.post("/create-coupon", {
        couponCode: String(data.get("couponCode")).trim().toUpperCase(),
        couponDiscount: Number(data.get("couponDiscount")),
        couponDescription: String(data.get("couponDescription")).trim(),
      });
      await refetch();
      form.reset();
