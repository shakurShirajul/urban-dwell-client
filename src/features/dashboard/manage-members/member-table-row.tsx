...
const MemberTableRow = ({ member, onRemove }: { member: UrbanUser; onRemove: (member: UrbanUser) => void }) => (
  <tr className="border-b border-base-content/10 last:border-0">
    <td className="px-4 py-4">
      <div className="flex items-center gap-3">
        {member.user_image ? <Image src={member.user_image} alt="" width="48" height="48" className="size-12 rounded-2xl object-cover" /> : <span className="grid size-12 place-items-center rounded-2xl bg-primary text-primary-content">{member.user_name?.charAt(0)}</span>}
