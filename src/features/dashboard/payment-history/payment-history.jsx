...
const PaymentHistory = () => {
  ...
  const search = (event) => {
    ...
    const value = normalizeMonth(event.currentTarget.month.value);
    value ? setSearchParams({ month: value }) : setSearchParams({});
  };
  ...
  return (
    <div>
      ...
      <form onSubmit={search} className="mb-6 flex max-w-xl gap-2" role="search">
        ...
        {month ? <button type="button" onClick={() => setSearchParams({})} className="grid size-[3.25rem] place-items-center rounded-xl border border-base-content/15" aria-label="Clear month filter"><HiOutlineXMark aria-hidden="true" /></button> : null}
      </form>
